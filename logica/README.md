...
## Parte 8 - Programa de Classificação de Animais

### Regras fornecidas:

1. Animal que tem pena não é mamífero  
   `∀x (TemPena(x) → ¬Mamifero(x))`

2. Animal que tem pelo é mamífero e não é ave  
   `∀x (TemPelo(x) → (Mamifero(x) ∧ ¬Ave(x)))`

3. Animal que não é ave e voa é morcego  
   `∀x ((¬Ave(x) ∧ Voa(x)) → Morcego(x))`

4. Animal que não tem pena tem pelo  
   `∀x (¬TemPena(x) → TemPelo(x))`

5. Sabiá é um animal que voa e não é mamífero  
   `Voa(sabia) ∧ ¬Mamifero(sabia)`

6. Pinguim é um animal que é ave e não voa  
   `Ave(pinguim) ∧ ¬Voa(pinguim)`

7. Vaca é um animal que é mamífero e não voa  
   `Mamifero(vaca) ∧ ¬Voa(vaca)`

8. Morcego é um animal que voa e não é ave  
   `Voa(morcego) ∧ ¬Ave(morcego)`

---

### Consulta: Identificar um animal que voa e não tem pena

**Queremos**: Encontrar `x` tal que `Voa(x) ∧ ¬TemPena(x)`

**Prova por dedução/resolução:**

- De (4): `¬TemPena(x) → TemPelo(x)`
- De (2): `TemPelo(x) → Mamifero(x) ∧ ¬Ave(x)`
- De (3): `(¬Ave(x) ∧ Voa(x)) → Morcego(x)`

Vamos usar `morcego`:

- Sabemos: `Voa(morcego) ∧ ¬Ave(morcego)` → `Morcego(morcego)`
- Pelo (3): `(¬Ave(morcego) ∧ Voa(morcego)) → Morcego(morcego)`
- Pelo (2): Se tem pelo, é mamífero
- Pelo (4): Se não tem pena → tem pelo
- Como morcego não é ave, pela (1), ele não tem pena
- Logo, `¬TemPena(morcego)` e `Voa(morcego)` → ✅

### ✅ Conclusão:
**O animal que voa e não tem pena é o morcego.**
